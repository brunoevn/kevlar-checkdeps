...
def print_banner():
    ...
    print(
        f"   {COLOR_GREEN}{ICON_SHIELD}  Kevlar Vulnerability Suppressions Configuration Wizard v{VERSION}{COLOR_RESET}"
    )
    print(
        f"                                                             {COLOR_GRAY}by Bruno Nielsen{COLOR_RESET}"
    )
    print(
        f"{COLOR_CYAN}{COLOR_BOLD}================================================================================{COLOR_RESET}\n"
    )
    print(
        f"This wizard helps you generate or update {COLOR_BOLD}kevlar-suppressions.json{COLOR_RESET} safely."
    )
    print(
        "It parses your generated scan report to let you select which alerts to suppress."
    )
...
def prompt_choice(prompt_text, options, default=None):
    ...
    for idx, opt in enumerate(options, 1):
        default_indicator = (
            f" {COLOR_GREEN}[default]{COLOR_RESET}" if default == str(idx) else ""
        )
    ...
    while True:
        ...
        print(
            f"{COLOR_RED}{ICON_ERROR} Invalid selection. Please enter a number between 1 and {len(options)}.{COLOR_RESET}"
        )


def prompt_string(
    prompt_text, default=None, min_len=0, custom_validator=None, validator_err_msg=None
):
    ...
    while True:
        ...
        if len(val) < min_len:
            print(
                f"{COLOR_RED}{ICON_ERROR} Input is too short. Minimum length is {min_len} characters.{COLOR_RESET}"
            )
            continue
...
def validate_suppressions_schema(data):
    ...
    for req_meta in ["version", "last_modified", "approved_by"]:
        if (
            req_meta not in metadata
            or not isinstance(metadata[req_meta], str)
            or not metadata[req_meta].strip()
        ):
            raise ValueError(f"Metadata field '{req_meta}' must be a non-empty string.")

    if not re.match(r"^\d+\.\d+(\.\d+)?$", metadata["version"].strip()):
        raise ValueError(
            "Metadata version format is invalid. Must be 'X.Y' or 'X.Y.Z'."
        )
    ...
    for idx, rule in enumerate(suppressions):
        for req_field in ["id", "package", "reason", "justification", "expires_at"]:
            if (
                req_field not in rule
                or not isinstance(rule[req_field], str)
                or not rule[req_field].strip()
            ):
                raise ValueError(
                    f"Rule {idx} is missing required non-empty string: '{req_field}'"
                )
        ...
        try:
            ...
            raise ValueError(
                f"Rule {idx} contains an invalid expiration date: '{expires_at}'"
            )
        ...
        for opt in ["ecosystem", "created_by", "approved_by"]:
            if opt in rule and rule[opt] is not None:
                if not isinstance(rule[opt], str) or not rule[opt].strip():
                    raise ValueError(
                        f"Optional field '{opt}' in rule {idx} must be a non-empty string."
                    )
...
def main():
    ...
    while True:
        ...
        if not os.path.exists(path_input):
            print(
                f"{COLOR_RED}{ICON_ERROR} File not found at: {path_input}{COLOR_RESET}"
            )
            # Scan for other .json files in current directory to help the user
            json_files = [f for f in os.listdir(".") if f.endswith(".json")]
            if json_files:
                print(
                    f"Found other JSON files in current directory: {', '.join(json_files)}"
                )
            continue
        ...
        try:
            ...
            if not isinstance(report_data, list):
                print(
                    f"{COLOR_RED}{ICON_ERROR} Invalid format: report.json must contain a list of packages.{COLOR_RESET}"
                )
                continue
            ...
            print(
                f"{COLOR_RED}{ICON_ERROR} Failed to parse JSON report file: {e}{COLOR_RESET}"
            )
    ...
    print("-----------------------------------------------")
    print(
        f"{COLOR_GRAY}{'No.':<5} {'Package':<25} {'Ecosystem':<12} {'Vulnerability ID':<18} {'Severity':<10} {'Summary':<40}{COLOR_RESET}"
    )
    ...
    for idx, item in enumerate(vulnerable_items, 1):
        ...
        print(
            f"[{idx:<2}] {pkg_display:<25} {item['technology']:<12} {item['vuln_id']:<18} {item['severity']:<10} {summary_display:<40}"
        )
    ...
    while True:
        ...
        print(
            f"{COLOR_RED}{ICON_ERROR} Invalid selection format. Enter numbers matching the list, e.g., '1, 2', '1-3' or 'all'.{COLOR_RESET}"
        )
    ...
    for count, idx in enumerate(selected_indices, 1):
        ...
        print(
            f"Configuring suppression {count} of {len(selected_indices)}: {COLOR_BOLD}{item['package']}{COLOR_RESET} ({item['vuln_id']})"
        )
        print(
        ...
        created_by = prompt_string(
            "Created by (author name/username) [Enter to skip]", default=""
        )
        approved_by = prompt_string(
            "Approved by (reviewer name/username) [Enter to skip]", default=""
        )
    ...
    default_target = (
        os.path.join(report_dir, "kevlar-suppressions.json")
        if report_dir
        else "kevlar-suppressions.json"
    )

    print(f"Target suppression policy file: {COLOR_BOLD}{default_target}{COLOR_RESET}")
    target_file = prompt_string(
        "Confirm or customize target file path", default=default_target
    )
    existing_data = None
    merge_mode = False
    ...
    if os.path.exists(target_file):
        print(
            f"{COLOR_YELLOW}{ICON_WARN} Found an existing '{target_file}' policy file.{COLOR_RESET}"
        )
        ...
        if save_choice == "1":
            try:
                ...
                print(
                    f"{COLOR_RED}{ICON_ERROR} Failed to load existing file. Defaulting to overwrite. Error: {e}{COLOR_RESET}"
                )
        ...
        for new_rule in new_suppressions:
            ...
            if duplicate_idx != -1:
                ...
                if confirm.lower() in {"y", "yes"}:
                    ...
                    merged_count += 1
                    print(
                        f"{COLOR_GREEN}{ICON_OK} Overwrote existing rule.{COLOR_RESET}"
                    )
                else:
                    print(
                        f"{COLOR_GRAY}{ICON_INFO} Skipped merging this rule.{COLOR_RESET}"
                    )
            else:
